  ...
  <a href="{{ url('/barang/'.$barang->id.'/peminjaman_history') }}" class="btn btn-info btn-xs">
      ...
      <table class="table table-bordered detail-view" style="text-transform: uppercase">
        ...
        <tr>
        <th style="width: 300px;">IMB (TGL/NOMOR/INSTANSI)</th><td style="width: 300px;"><b>: </b>{{ $barang->properties->imb->nomor ?? 'null'}} {{ $barang->properties->imb->tanggal ?? 'null'}} {{ $barang->properties->imb->instansi ?? 'null'}}</td>
